...
import { Heart, User, Menu, X, ChevronDown, Bell } from 'lucide-react'
...
export default function Navbar({ onLogoClick, onSellClick, onDashboardClick, onNotificationClick, onFavoritesClick, activePage }) {
        ...
        <div className="max-w-container mx-auto px-5 lg:px-10 h-16 flex items-center justify-between gap-6">
          ...
          <div className="hidden md:flex items-center gap-2">
            <button
              ...
              aria-label="Notifications / Leads"
            ...
            <button
              ...
              className="relative p-2 rounded text-on-surface-variant hover:text-deep-navy hover:bg-surface-container transition-colors"
            >
              <Heart size={20} className={favoritesCount > 0 ? "text-accent-red fill-accent-red" : ""} />
              {favoritesCount > 0 && (
                <span className="absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full bg-accent-red text-white text-[10px] font-bold flex items-center justify-center shadow-sm">
                  {favoritesCount}
                </span>
              )}
            </button>
          ...
          <div className="flex md:hidden items-center gap-2">
            ...
            <button
              onClick={() => onFavoritesClick?.()}
              aria-label="Saved cars"
              className="relative p-2 rounded text-on-surface-variant hover:text-deep-navy transition-colors"
            >
              <Heart size={20} className={favoritesCount > 0 ? "text-accent-red fill-accent-red" : ""} />
              {favoritesCount > 0 && (
                <span className="absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full bg-accent-red text-white text-[10px] font-bold flex items-center justify-center shadow-sm">
