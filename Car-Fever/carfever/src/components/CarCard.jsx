...
const DEFAULT_FALLBACK = 'https://images.unsplash.com/photo-1541443131876-44b03de101c5?w=800&q=80&auto=format&fit=crop'

export default function CarCard({ car, onViewDetails }) {
  const { isFavorite, toggleFavorite } = useFavorites()
  const saved = isFavorite(car.id)

  const monthlyFinance = Math.round((car.price || 0) / 48 * 1.08)
    ...
    >
      ...
      <div className="relative overflow-hidden aspect-[16/10] bg-surface-container-low">
        <img
          ...
          loading="lazy"
          onError={(e) => {
            if (e.target.src !== DEFAULT_FALLBACK) {
        ...
        <div className="absolute top-2.5 left-2.5 flex flex-wrap gap-1">
          {Array.isArray(car.badges) && car.badges.map(badge => (
            <span
              key={badge}
        ...
        <button
          ...
          className={`
            absolute top-2.5 right-2.5 p-1.5 rounded-full backdrop-blur-sm transition-all duration-150 shadow-md
            ${saved
